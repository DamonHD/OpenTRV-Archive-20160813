...
symbol CLOCK_SPEED_MAX = m16 ; As fast as possible while maintaining elapsed 'time' word correctly on 18M2+ and works down to minimum supply voltage.
#else
symbol CLOCK_SPEED_MAX = m32 ; As fast as possible for 18M2+.  Not OK at 1.8V.
#endif
#endif
#endif
...
; Pulled high, so can be left as input.
#ifdef USE_MODULE_RFM22RADIOSIMPLE
; C.5: Active low to indicate interrupt from RFM22 (input from radio module).  Optional, else status will be polled in s/w.
; Pulled high, so can be left as input or set to high output.
;#define RFM22_nIRQ_I_PIN input5
;#define RFM22_nIRQ_I = C.5
#endif
...
mainLoop:
    ...
    ; If likely overrun already then shio low-priority jobs.
    ...
    if tempB0 != 0 then
        ...
        ; ONLY do this if not syncing with TRV and if this node is not itself not a boiler hub (with no one listening)
        ; and if this unit actually wants to call for heat to avoid shouting down nodes that do.
        if syncedWithFHT8V = 1 AND TRVPercentOpen != 0 then
...
#ifndef BOILER_HUB ; Extra callout from non-boiler nodes to boiler node.
#ifdef USE_MODULE_FHT8VSIMPLE_TX
        case 9,$26 ; Schedule randomised resends of wireless TRV / boiler call to help avoid collisions.
...
#ifdef USE_MODULE_DS1306RTCSPISIMPLE
        gosub DS1306ReadBCDSecondsB0
        tempB0 = B0 ; BCD seconds.
#else
        tempB0 = time ; Internal elapsed time measure least-significant byte (binary).
#endif
        if tempB0 != TIME_LSD then ; tempB0 is putative new value for TIME_LSD...
...
pollShowOrAdjustMode:
    ...
    if BUTTON_MODE = 1 then
        ...
        if isWarmMode = 0 then ; Was in FROST mode, move to WARM.
            ...
            high LED_HEATCALL
            gosub mediumPause
        else ; Was in BAKE, move to FROST.
            isWarmMode = 0
            gosub mediumPause    ; Medium flash 'heat call' to indicate now in frost mode.
        endif
        low LED_HEATCALL
        slowOpDone = 1 ; Pauses big enough to be trouble with timing...
        ...
        if isWarmMode = 1 then
            ...
            if TRVPercentOpen != 0 then
                ...
                if bakeCountdownM != 0 then ; Third flash if in 'bake' mode.
                    ...
                    high LED_HEATCALL ; flash
                    gosub tinyPause ; Sum of pauses must not take anything like full 1s slot time...
                    low LED_HEATCALL
                endif
            endif
        endif
...
#ifdef USE_MODULE_FHT8VSIMPLE_TX
; Unconditionally create FHT8V TRV outgoing valve-setting command frame (terminated with $ff) in FHT8VTXCommandArea.
...
; This is CPU intensive, and can be run at a high clock speed if required.
; Destroys: bptr, tempW0.
CreateFHT8VValveSetCmdFrame:
    bptr = FHT8VTXCommandArea
    goto FHT8VCreateValveSetCmdFrame ; GOSUB CHAIN: gosub FHT8VCreateValveSetCmdFrame return ; For speed and to preserve gosub slots.
...
#ifdef IGNORE_FHT_SYNC ; Dumb and wasteful (and not legal in terms of ISM duty cycle) but can help with debugging
...
#endif
    gosub FHT8VTXFHTQueueAndTwiceSendCmd
#else ; Use FHT8V sync procedure and keep duty cycle low...
...
; RECENT CHANGES
; DHD20130312: FHT8V sync working, but currently on 2037 out of 2048 program bytes!
...
; Reads the seconds register (in BCD, from $00 to $59) into B0.
; Minimal SPI interaction to do this, so reasonably quick.
