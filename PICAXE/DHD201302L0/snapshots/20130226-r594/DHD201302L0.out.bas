...
; the unit generates one or two short flashes on a two-second cycle if in heat mode.
...
; IF DEFINED: this unit may run on 2xAA cells, preferably rechargable eg NiMH, ~2V--2.4V, so may need to monitor supply voltage with calibadc.
...
#define LOCAL_TRV
; A couple of my valves (45,26), (13,73).
...
isRoomLit = 1 ; Start up assuming that the room is lit; stays this way if LDR support omitted.
;
; Boolean temporary.
...
do
    ...
    if tempB0 != 0 then
        ...
        if isWarmMode = 1 AND TRVPercentOpen != 0 then
    ...
    if tempB0 != 0 then
        ...
        if TRVPercentOpen = 0 then ; Stop calling for heat from the boiler.
            low OUT_HEATCALL
...
computeTargetAndDemand:
    ...
    if isWarmMode = 0 then
        ...
        if isRoomLit = 1 then
            ...
            targetTempC = WARM - SETBACK ; Target must never be below FROST.
        end if
    end if
