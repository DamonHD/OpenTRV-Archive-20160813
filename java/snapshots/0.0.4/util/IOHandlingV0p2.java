...
public final class IOHandlingV0p2
    ...
    /**Minimum interval in seconds between temperature stats (for a given sensor) entries unless the temp changes; strictly positive.
...
plot "sample.log" using 1:3 with linespoints
    ...
    /**Sub-directory of stats dir for recording remote temperatures from directly-attached OpenTRV unit. */
    public static final String remoteTempSubdir = "remoteTemp";

    /**Map from ID to last remote temperature record written for that ID, including time; non-null. */
...
@A45;T21CC
    ...
    private void processRemoteStats(final String line, final File statsDir)
        ...
        if("".equals(parsed.ID)) { return; } // Skip record with empty ID.
        final String rawTemp = parsed.sectionsByKey.get(ParsedRemoteStatsRecord.KEY_TEMPERATURE);
        if(null == rawTemp) { return; } // Skip record with missing temperature.

        final Date now = new Date(parsed.constructionTime);
        final File dir = new File(statsDir, remoteTempSubdir);
        final File logFile;
        synchronized(dateForFilename) { logFile = new File(dir, dateForFilename.format(now) + ".log"); }
        ...
        if((null != lw) &&
           rawTemp.equals(lw.sectionsByKey.get(ParsedRemoteStatsRecord.KEY_TEMPERATURE)) &&
    ...
    /**UTC date-only format for filenames; hold lock on instance while using for thread-safety. */
    ...
    /**UTC full time and date format; hold lock on instance while using for thread-safety. */
